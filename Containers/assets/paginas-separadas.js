...
// A data da saída é a data/hora registrada para o movimento de SAÍDA.
// Mantém o Firebase como fonte de dados e não cria um novo campo obrigatório.
function dataSaidaMercadoria(x){
  if(!x) return "—";
  const tipo=tipoMovimentoNormalizado(x);
  const dataExplicita=x.dataSaida||x.dataSaída||x.saidaEm||x.saidaAt;
  if(dataExplicita) return fmtData(dataExplicita);
  return tipo==="SAÍDA" ? fmtData(dataMovimentacao(x)) : "—";
}
...
function configuracaoRelatorio(tipo){
  if(tipo==="movimentacoes") return {
    ...
    headers:["Data/Hora","Data de Saída","Tipo","Container","Produto","Código","Lote","Qtd.","Peso","NF","Operador","Detalhes"],
    valores:x=>[fmtData(dataMovimentacao(x)),dataSaidaMercadoria(x),x.tipo||"—",x.num||x.container||"—",x.prod||x.produto||"—",x.codigo||"—",x.lote||"—",x.qtd??"—",x.peso??x.kg??"—",x.nf||"—",x.operador||x.usuario||"—",x.detalhes||"—"]
  };
  if(tipo==="registros") return {
    titulo:"📥 Registros de Entrada e Saída",
    headers:["Data/Hora","Entrada","Saída","Data de Saída","Tipo","Container","Produto","Código","Lote","Qtd.","Peso","NF","Fornecedor","Operador"],
    valores:x=>[fmtData(dataMovimentacao(x)),quantidadeMovimento(x,"ENTRADA"),quantidadeMovimento(x,"SAÍDA"),dataSaidaMercadoria(x),x.tipo||"—",x.num||x.container||"—",x.prod||x.produto||"—",x.codigo||"—",x.lote||"—",x.qtd??"—",x.peso??x.kg??"—",x.nf||"—",x.fornecedor||"—",x.operador||x.usuario||"—"]
  };
  return {
    titulo:"📨 Solicitações de Usuários Externos",
    headers:["Data/Hora","Data de Saída","Status","Container","Produto","Código","Lote","Qtd.","Peso","Solicitante","Filial","NF"],
    valores:x=>[fmtData(x.dataHora||x.data||x.criadaEm),dataSaidaMercadoria(x),x.status||"pendente",x.container||x.num||x.containerOrigem||"—",x.produto||x.prod||"—",x.codigo||"—",x.lote||"—",x.quantidade??x.qtd??"—",x.peso??x.kg??"—",x.solicitante||x.nome||x.usuario||x.email||"—",x.filial||"—",x.nf||x.notaFiscal||"—"]
  };
}
...
function linhaKardex(origem,x,sourceIndex){
  ...
  return {
    ...
    data:fmtData(dataMovimentacao(x)),dataSaida:dataSaidaMercadoria(x),_dataOrdenacao:valorDataOrdenacao(dataMovimentacao(x)),
...
function renderizarKardexTabela(lista){
  ...
  const headers=['Origem','Data/Hora','Entrada','Saída','Data de Saída','Tipo/Status','Container','Produto','Código','Lote','Qtd.','Peso','NF','Solicitante/Operador','Filial','Detalhes'];
  ...
  body.innerHTML=lista.map((x,idx)=>{const vals=[x.origem,x.data,x.entrada,x.saida,x.dataSaida,x.tipo,x.container,x.produto,x.codigo,x.lote,x.qtd,x.peso,x.nf,x.solicitante,x.filial,x.detalhes];return '<tr>'+vals.map(v=>'<td style="padding:7px;border-bottom:1px solid var(--borda);">'+esc(v)+'</td>').join('')+'<td><button class="btn-acao" title="Apagar linha" onclick="excluirLinhaKardex('+idx+')">🗑️</button></td></tr>';}).join('');
