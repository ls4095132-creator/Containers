...
(function () {
  ...
  const BASE_URL = "https://ls4095132-creator.github.io/Containers/";
  const pagina = String(document.body?.dataset?.pagina || "").trim().toLowerCase();

  function urlSistema(caminho) {
    return new URL(String(caminho || "").replace(/^\/+/, ""), BASE_URL).href;
  }

  const ROTAS = {
    containers: urlSistema("containers/"),
    relatorios: urlSistema("relatorios/"),
    kardex: urlSistema("kardex/"),
    produtos: urlSistema("produtos/"),
    usuarios: urlSistema("usuarios/"),
    configuracao: urlSistema("config/configuracao.html?aba=acesso")
  };
  ...
  // Quando o administrador libera uma tela para um usuário comum ou externo,
  // a liberação vale também para as ações disponíveis dentro daquela tela.
  window.usuarioPodeOperarTela = function(nomeTela) {
    return podeAcessarTela(obterTipoAtual(), nomeTela, obterAcessosAtual());
  };
  ...
  function aplicarMenu() {
    ...
    document.querySelectorAll("#lista-menu-principal .item-menu-principal[data-tela]").forEach(function(btn) {
      ...
      // Tela bloqueada não aparece no menu do usuário. Ao liberar no Firebase,
      // ela volta automaticamente para o menu na próxima sincronização.
  ...
  window.abrirConfiguracaoUsuariosSeparada = function () {
    ...
    window.location.href = urlSistema("config/configuracao.html?aba=acesso");
  };
  ...
  async function protegerPagina() {
    ...
    if (!podeAcessarTela(tipo, pagina, obterAcessosAtual())) {
      alert("🔒 Você não tem permissão para acessar esta tela.");
      window.location.replace(urlSistema(""));
      return;
    }
  ...
  window.abrirRelatorioMovimentacoes = function () {
    ...
    window.location.href = urlSistema("containers/#historico");
  };
  window.abrirRelatorioRegistros = function () {
    if (obterTipoAtual() !== TIPOS.ADMIN) { alert("🔒 Apenas o administrador pode acessar este relatório."); return; }
    window.location.href = urlSistema("containers/#registro-mercadoria");
  };
  window.abrirRelatorioSolicitacoes = function () {
    if (obterTipoAtual() !== TIPOS.ADMIN) { alert("🔒 Apenas o administrador pode acessar este relatório."); return; }
    window.location.href = urlSistema("containers/#solicitacoes");
  };
  ...
  if (typeof window.sairDoSistema === "function") {
    ...
    window.sairDoSistema = async function () {
      await sairOriginal();
      setTimeout(function(){ window.location.href = urlSistema(""); }, 120);
    };
  }
