...
NOTIFICAÇÕES
- OneSignal foi descontinuado do sistema atual.
- Os scripts CDN do OneSignal foram removidos das páginas ativas.
- A integração OneSignal do sistema.js foi removida.
- As notificações Web continuam utilizando Firebase Cloud Messaging (FCM).
- Na abertura do sistema, registros antigos do Service Worker OneSignal são
  desregistrados quando encontrados, para eliminar o erro 404 antigo.


ATUALIZAÇÃO 23/09/2026
- OneSignal descontinuado.
- Firebase Cloud Messaging (FCM) mantido.
- Navegação separada corrigida para exibir a tela autorizada após login.
- Rotinas exclusivas de Containers não são executadas em Relatórios/Kardex/Produtos/Usuários.
- Todas as telas separadas possuem retorno ao Menu Principal sem encerrar a sessão.
- Botões globais Atualizar/Configuração/Sair ficam no Menu Principal.
