...
function linhaKardex(origem,x,sourceIndex){
  return {
    ...
    data:fmtData(x.dataHora||x.data||x.criadaEm||x.processadoEm),_dataOrdenacao:String(x.dataHora||x.data||x.criadaEm||x.processadoEm||''),
...
async function carregarKardexCompleto(){
  ...
  lista.sort((a,b)=>{const db=Date.parse(b._dataOrdenacao),da=Date.parse(a._dataOrdenacao);if(Number.isFinite(db)&&Number.isFinite(da))return db-da;return String(b._dataOrdenacao).localeCompare(String(a._dataOrdenacao));}); kardexCompleto=lista; return lista;
}
