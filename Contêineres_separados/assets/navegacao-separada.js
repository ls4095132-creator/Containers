...
(function () {
  ...
  function obterUsuarioCore() {
    try {
      if (typeof usuarioLogadoAtual !== "undefined" && usuarioLogadoAtual) return usuarioLogadoAtual;
    } catch (e) {}
    return null;
  }
  ...
  function podeAcessarTela(tipo, tela) {
    ...
    if (t === TIPOS.ADMIN) return ["containers","relatorios","kardex","produtos","usuarios","configuracao"].includes(telaNormalizada);
    if (t === TIPOS.USUARIO) return telaNormalizada === "containers" || telaNormalizada === "kardex";
    if (t === TIPOS.EXTERNO) return telaNormalizada === "containers";
    return false;
  }
