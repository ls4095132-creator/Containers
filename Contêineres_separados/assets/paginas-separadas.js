...
function configuracaoRelatorio(tipo){
  if(tipo==="movimentacoes") return {
    ...
    valores:x=>[fmtData(x.dataHora||x.data),x.tipo||"—",x.num||x.container||"—",x.prod||x.produto||"—",x.codigo||"—",x.lote||"—",x.qtd??"—",x.peso??x.kg??"—",x.nf||"—",x.operador||x.usuario||"—",x.detalhes||"—"]
  };
  if(tipo==="registros") return {
    ...
    valores:x=>[fmtData(x.dataHora||x.data),x.tipo||"—",x.num||x.container||"—",x.prod||x.produto||"—",x.codigo||"—",x.lote||"—",x.qtd??"—",x.peso??x.kg??"—",x.nf||"—",x.fornecedor||"—",x.operador||x.usuario||"—"]
  };
  return {
...
async function excluirLinhaRelatorio(posicao){
  if(!usuarioEhAdmin()){alert('🔒 Apenas o administrador pode apagar registros dos relatórios.');return;}
...
window.compartilharRelatorio=async function(){
  const dados=dadosRelatorioExportacao(); if(!dados.length){alert('Não há dados para compartilhar.');return;}
  let blob, nome, tipo='text/csv;charset=utf-8';
  if(window.XLSX){const ws=XLSX.utils.aoa_to_sheet([relatorioEstado.headers,...dados]);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Relatório');blob=new Blob([XLSX.write(wb,{bookType:'xlsx',type:'array'})],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});nome='Relatorio_'+relatorioEstado.tipo+'.xlsx';tipo=blob.type;}
  else {const obj=dados.map(r=>Object.fromEntries(relatorioEstado.headers.map((h,i)=>[h,r[i]])));blob=new Blob([gerarCSV(obj)],{type:tipo});nome='Relatorio_'+relatorioEstado.tipo+'.csv';}
  const file=new File([blob],nome,{type});
  if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){try{await navigator.share({title:configuracaoRelatorio(relatorioEstado.tipo).titulo,text:'Relatório — Controle de Containers',files:[file]});return;}catch(e){if(e.name==='AbortError')return;}}
  baixarArquivo(blob,nome,blob.type);
  alert('O compartilhamento nativo não está disponível neste navegador. O arquivo foi baixado.');
};
...
function linhaKardex(origem,x,sourceIndex){
  return {
    ...
    tipo:x.tipo||x.status||origem,container:x.num||x.container||x.containerOrigem||x.containerDestino||'—',
...
async function carregarKardexCompleto(){
  ...
  lista.sort((a,b)=>String(b.data).localeCompare(String(a.data))); kardexCompleto=lista; return lista;
}
function renderizarKardexTabela(lista){
  ...
  body.innerHTML=lista.map((x,idx)=>{const vals=[x.origem,x.data,x.tipo,x.container,x.produto,x.codigo,x.lote,x.qtd,x.peso,x.nf,x.solicitante,x.filial,x.detalhes];return '<tr>'+vals.map(v=>'<td style="padding:7px;border-bottom:1px solid var(--borda);">'+esc(v)+'</td>').join('')+'<td><button class="btn-acao" title="Apagar linha" onclick="excluirLinhaKardex('+idx+')">🗑️</button></td></tr>';}).join('');
  if(vazio)vazio.style.display=lista.length?'none':'block';
}
window.renderizarKardex=async function(){try{const lista=await carregarKardexCompleto();renderizarKardexTabela(lista);}catch(e){console.error('Kardex:',e);}};
function dadosExportacaoKardex(){return kardexCompleto.map(x=>({'Origem':x.origem,'Data/Hora':x.data,'Tipo/Status':x.tipo,'Container':x.container,'Produto':x.produto,'Código':x.codigo,'Lote':x.lote,'Quantidade':x.qtd,'Peso':x.peso,'NF':x.nf,'Solicitante/Operador':x.solicitante,'Filial':x.filial,'Detalhes':x.detalhes}));}
async function garantirKardex(){if(!kardexCompleto.length)await window.renderizarKardex();return kardexCompleto;}
window.excluirLinhaKardex=async function(idx){
  if(!usuarioEhAdmin()){alert('🔒 Apenas o administrador pode apagar o Kardex.');return;}
  const item=kardexCompleto[idx];if(!item)return;
  if(!confirm('Apagar esta linha do Kardex?\n\nA ação será aplicada ao registro correspondente no Firebase.'))return;
  try{
    const path=item.origem==='HISTÓRICO'?'historico':item.origem==='SOLICITAÇÃO EXTERNA'?'solicitacoesMercadoria':'estoque';
    const snap=await firebaseDB.ref(path).once('value');const lista=arr(snap.val());let pos=-1;
    if(item.id)pos=lista.findIndex(x=>x&&String(x.id)===String(item.id));
    if(pos<0)pos=item.sourceIndex;
    if(pos<0||pos>=lista.length)throw new Error('Linha não encontrada no Firebase.');
    lista.splice(pos,1);await firebaseDB.ref(path).set(lista);await window.renderizarKardex();alert('✅ Linha apagada do Kardex.');
  }catch(e){console.error(e);alert('❌ Não foi possível apagar a linha.\n\n'+(e.message||e));}
};
