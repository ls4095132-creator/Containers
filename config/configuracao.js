...
async function definirTemaAdmin(tema){
    const t=String(tema||"escuro").toLowerCase()==="claro"?"claro":"escuro";
    aplicarTema(t);
    if(!usuarioAtual)return;
    try{
        await db.ref("preferenciasUsuarios/"+usuarioAtual.uid+"/tema").set(t);
        mensagem("✅ Tema do administrador alterado para "+(t==="claro"?"claro":"escuro")+".");
    }catch(e){console.error(e);mensagem("❌ Não foi possível salvar o tema do administrador.",true);}
}
...
async function carregarConfiguracoes(){
    ...
    let temaAdmin=String(configuracoes.tema||"escuro");
    try{
        if(usuarioAtual){
            const ts=await db.ref("preferenciasUsuarios/"+usuarioAtual.uid+"/tema").once("value");
            if(ts.exists() && ts.val()) temaAdmin=String(ts.val());
        }
    }catch(e){}
    aplicarTema(temaAdmin);
