...
async function carregarTemaAdministrador(){
    try{
        if(!auth.currentUser) return;
        const snap=await db.ref("preferenciasUsuarios/"+auth.currentUser.uid+"/tema").once("value");
        const tema=(snap.exists() && snap.val()) ? String(snap.val()).toLowerCase() : "escuro";
        const t=tema === "claro" ? "claro" : "escuro";
        document.documentElement.setAttribute("data-tema", t);
        document.body.classList.toggle("tema-claro", t === "claro");
    }catch(e){
        console.warn("Não foi possível carregar o tema individual do administrador:",e);
    }
}
...
function iniciar(){
    ...
    auth.onAuthStateChanged(async user=>{
        ...
        try{
            ...
            validado=true;
            await carregarTemaAdministrador();
